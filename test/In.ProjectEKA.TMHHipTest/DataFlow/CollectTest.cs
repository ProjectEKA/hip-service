namespace In.ProjectEKA.TMHHipTest.DataFlow
{
    ...
    using TMHHip.DataFlow.Model;
    using TMHHip.Link;
    using Xunit;
    using Patient = TMHHip.Discovery.Patient;
    ...
    public class CollectTest
    {
        ...
        private async void ReturnDiagnosticReportAsPdfWithUrlRedirect()
        {
            ...
            var diagReport = new DiagnosticReportAsPdf
            {
                ...
                ReportTitle = "title",
                ReportUrl =
                    "https://tmc.gov.in/EMRREPORTS1/allreports1.aspx?reqno=JfMcCHl8H2Yx6CfwfckII4IUQzG9QChzqJQ8BGLQPdw5HGovDusYmyKH1gbRQc8t",
                EffectiveDateTime = dateObj
            };
            var diagReports = new List<DiagnosticReportAsPdf> {diagReport};

            var patientDataResponse =
                JsonConvert.SerializeObject(new PatientData {DiagnosticReportAsPdfs = diagReports});
        ...
        [Fact]
        private async void ReturnDiagnosticReportAsImage()
        {
            var handlerMock = new Mock<HttpMessageHandler>(MockBehavior.Loose);
            var repoMock = new Mock<HttpMessageHandler>(MockBehavior.Loose);
            var tmhClient = new HttpClient(handlerMock.Object);
            ...
            var reportAsImages = new List<DiagnosticReportAsImage> {reportAsImage};

            var patientDataResponse =
                JsonConvert.SerializeObject(new PatientData {DiagnosticReportAsImages = reportAsImages});
